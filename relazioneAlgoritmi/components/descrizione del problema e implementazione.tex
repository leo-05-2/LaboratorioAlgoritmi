...
successivamente verranno confrontate con opportuni esperimenti le prestazioni nelle operazioni select(k) e rank(x) ogni struttura dati.
...
sono state implementate tre diverse strutture dati per supportare queste operazioni: una lista concatenata ordinata, un albero binario di ricerca standard e un albero binario di ricerca auto bilanciato (AVL) con attributo dimensione.
...
Ogni nodo è un oggetto e ha 4 campi: valore(la chiave dell' nodo quindi il dato), puntatore al figlio sinistro, puntatore al
...
La classe \texttt{SBSTree} implementa basato su una struttura \textbf{AVL (Albero Autobilanciato)}.\\
...
\item \textbf{rank(x)}: Implementata scendendo l'albero e calcolando ricorsivamente la dimensione di sottoalberi sinistri.
\end{itemize}
