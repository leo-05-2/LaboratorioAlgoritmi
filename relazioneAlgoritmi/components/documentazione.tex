...
I metodi principali della linked list oltre a select e rank:
...
la classe di misurazione è organizzata per ottenere stime affidabili delle prestazioni mediando molteplici misure su dataset indipendenti.
In pratica, per ogni dimensione di interesse si generano più dataset (sia casuali che ordinati a seconda dell' esperimento), si popola la struttura in prova e si esegue una sequenza di campionamenti delle operazioni \texttt{select(k)} e \texttt{rank(x)}.
...
\begin{description}
    \item [\texttt{plot\_results(flag: Boolean, structure\_names, random: Boolean)}] Genera grafici comparativi delle performance per le strutture specificate, distinguendo tra i tipi di dataset (casuali o degenerati) con il parametro \texttt{random}, \\
    il parametro \texttt{flag} indica se i grafici se si vuole visualizzare un grafico per ciascuna struttura dati singolarmente o se le strutture passate con \texttt{structure\_names} devono essere confrontate in un unico grafico, flag = False genera un grafico per ogni struttura dati, flag = true genera un unico grafico di confronto tra le strutture dati passate in \texttt{structure\_names}.

    \item[\texttt{run\_tests(random: Boolean)}] Esegue i test per tutte le strutture dati specificate, raccogliendo i risultati in modo organizzato, il parametro \texttt{random} indica se utilizzare dataset casuali o degenerati.
    % plot_results
\end{description}
...
\begin{itemize}
    ...
    \item \textbf{Iterazione su Dimensioni Diverse} (\texttt{sizes}): I test vengono eseguiti per un intervallo di dimensioni di input \texttt{n} (da 1 a 10.000), permettendo di osservare come le performance scalano al crescere dei dati.
...
\\
I risultati finali sono conservati: per ogni dimensione si mantengono media e deviazione standard sia per \texttt{select} sia per \texttt{rank}. StructureTester espone inoltre la possibilità di salvare i dati grezzi e di produrre grafici descrittivi con nomi di file costruiti automaticamente per facilitare l'analisi a posteriori.
\\
